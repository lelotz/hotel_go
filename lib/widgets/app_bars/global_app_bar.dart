...
PreferredSizeWidget buildGlobalAppBar(BuildContext context,{String appBarTitle = "Whitemark Hotels",Color statusColor = ColorsManager.primaryAccent,void Function()? onTitleTap}){
  ...
  return AppBar(
    toolbarHeight: 85 ,
    leading: IconButton(icon: const Icon(Icons.arrow_back_ios,color: ColorsManager.grey1,), onPressed: () {
      ...
      FittedBox(
        child: Row(
          ...
          children: [
            ...
            MyOutlinedButton(text: 'SW', onClick: langController.changeLocaleToSwahili,width: 80,height: 40,),
            MyOutlinedButton(text: 'EN', onClick: langController.changeLocaleToEnglish,width: 80,height: 40,),
            Padding(
              padding: const EdgeInsets.all(8.0),
              child: FittedBox(
                child: InkWell(
                  onTap: (){buildDialog(context,'',AdminCardPopUp(),alignment: Alignment.topRight);},
                  child: AdminUserCard(),
                ),
              ),
            ),
  ...
  Widget buildAppBarTitle(String title, {Color statusColor = ColorsManager.primaryAccent,void Function()? onTitleTap}){
  //print('$title ${title.length}');
    return Column(
      children: [
        InkWell(
          onTap: onTitleTap ?? (){},
            child: BigText(text: title,color: ColorsManager.white,)),
        Container(

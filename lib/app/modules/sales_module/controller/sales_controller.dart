...
import 'package:hotel_pms/widgets/tables/paged_data_table_source.dart';
...
class SalesController extends GetxController{
  ...
  Rx<DateTime> startDate = Rx<DateTime>(DateTime.now().add(const Duration(days: -1)));
  ...
  DataGridSource? saleTableSource;
  ...
  void exportSalesTable(GlobalKey<SfDataGridState> salesTableKey,{bool toExcel=true})async{
    if(toExcel){
      isExporting.value = true;
      await ExportTableData().exportDataGridToExcel(key: salesTableKey, filePath: await generateSalesFileName(),fileCategory: '');
      isExporting.value = false;
    }
  }

  Future<String> generateSalesFileName()async{
    String subfolder = extractDate(DateTime.now().add(Duration(days: random(1,  10)))).replaceAll('-', '_');
    String fileName = loggedInUser.fullName!;
  ...
  void setUpTableSource(){
    saleTableSource = SyncFusionDataSource(
        sourceData: List<DataGridRow>.generate(collectedPayments.value.length, (index) {
          return DataGridRow(cells: [
            DataGridCell<int>(columnName: '#', value: index),
            DataGridCell<int>(columnName: 'ROOM NUMBER', value: collectedPayments.value[index].roomNumber),
            DataGridCell<String>(columnName: 'DATE', value: collectedPayments.value[index].date!),
            DataGridCell<String>(columnName: 'TIME', value: collectedPayments.value[index].time!),
            DataGridCell<String>(columnName: 'EMPLOYEE', value: collectedPayments.value[index].employeeName),
            DataGridCell<String>(columnName: 'CLIENT', value: collectedPayments.value[index].clientName),
            DataGridCell<String>(columnName: 'SERVICE', value: collectedPayments.value[index].service!),
            DataGridCell<int>(columnName: 'COLLECTED', value: collectedPayments.value[index].amountCollected),
            DataGridCell<String>(columnName: 'PAY METHOD', value: collectedPayments.value[index].payMethod!),
          ]);
        }),
    );
  ...
  validateSearchCategory(String category){
    if(searchCategory.value == LocalKeys.kSelectSearchCategory) return "Search Category cannot be empty";
    return null;
  }
    ...
    await SqlDatabase.instance.read(tableName: AdminUsersTable.tableName,readAll: true).then((value) {
      if(value!=null && value.isNotEmpty){
        for(Map<String,dynamic> element in value){
          employees.value.addIf(employees.value.contains(element['fullName']) == false,element['fullName']);
        }
      }
     employees.refresh();
    });
  ...
  getAllPaymentTransactions()async{
    ...
    await SqlDatabase.instance.read(
        tableName: CollectedPaymentsTable.tableName,readAll: true).then((value) {
      if(value != null && value.isNotEmpty){
        for(Map<String,dynamic> element in value){
      ...
      filterResultStatus.value = "Matches : ${value!.length} Displaying : ${collectedPaymentsCount.value}";
    });
  }

  getTodaysTransactions()async{
      await CollectedPaymentsRepository().getCollectedPaymentsByDate(extractDate(DateTime.now())).then((value) {
        if(value != null && value.isNotEmpty){
  ...
  getAmountCollectedToday()async{
    for(CollectPayment element in collectedPayments.value){
      amountCollectedToday.value = amountCollectedToday.value + element.amountCollected!;
    }
  }
  ...
  filterCollectedPaymentsByCategory()async{
    String sql = " ${searchCategory.value} = ? AND ${CollectedPaymentsTable.dateTime} BETWEEN ? AND ? ";
    List<Object?> whereArgs = [
      searchCtrl.text.length == 3 ? stringToInt(searchCtrl.text) : searchCtrl.text,
    ...
    await SqlDatabase.instance.read(
      ...
      if(value != null && value.isNotEmpty){
        ...
        updateUI();
        logger.v({'title':'SUCCESS Category Filter ','sql':sql,'data_length': collectedPayments.value.length,'whereArgs':whereArgs});
      }else{
        logger.w({'title': 'No results from Category Filter query', 'sql': sql,'whereArgs':whereArgs});
      }
      filterResultStatus.value = "Matches :${value!.length} Displaying : ${collectedPaymentsCount.value}";
    });
    setUpTableSource();
  ...
  filterCollectedPaymentsBlindSearch()async{
    ...
    await SqlDatabase.instance.read(
      ...
      collectedPayments.value = CollectPayment().fromJsonList(value);
      updateUI();
      logger.v({'title':'SUCCESS Blind Filter ','sql':sql,'data_length': collectedPayments.value.length,'whereArgs':whereArgs});
      }else{
        logger.w({'title': 'No results from blind query', 'sql': sql,'whereArgs':whereArgs});
      }
  ...
  filterCollectedPayments()async{
    ...
    Map<String,dynamic> filters = buildFilterSql(
        [
          {'name': CollectedPaymentsTable.employeeName,'value':selectedEmployeeFilter.value},
          {'name': CollectedPaymentsTable.roomNumber,'value': stringToInt(roomNumberCtrl.text)},
          {'name': CollectedPaymentsTable.service,'value':selectedServiceFilter.value},
          {'name': CollectedPaymentsTable.payMethod,'value':selectedPayMethodFilter.value},
          {'name':CollectedPaymentsTable.clientName, 'value': guestNameCtrl.text}
        ],
      resetTimeInDateTime(startDate.value),resetTimeInDateTime(endDate.value,toEndOfDay: true)

      // startDate.value.add(const Duration(days: -1)).toIso8601String(),endDate.value.add(const Duration(days: 1)).toIso8601String()
    );
  ...
  Map<String,dynamic> buildFilterSql(List<Map<String,dynamic>> variables,String startDate,String endDate){
    String join = ' AND ';
    String where = "";
    List<Object?> whereArgs = [];
    ...
    for (Map<String, dynamic> element in variables){
      if(element['value'] != "" && element['value'] != 0) validVariables.add(element);
    }
    for(Map<String, dynamic> element in validVariables){
        ...
        if(validVariablesCount != validVariables.length && validVariablesCount != 0) where = where + join;
    }

    where = validVariables.isNotEmpty ? "$where$join${CollectedPaymentsTable.dateTime} BETWEEN ? AND ?" : "${CollectedPaymentsTable.dateTime} BETWEEN ? AND ?";
    whereArgs.add(startDate);
    whereArgs.add(endDate);
    selectedFilters.value.add("startDate ${extractDate(DateTime.parse(startDate))}");
    selectedFilters.value.add("startDate ${extractDate(DateTime.parse(endDate))}");
    updateUI();
    logger.i({'name': 'filtered sql parameters','where':where,'whereArgs':whereArgs});
